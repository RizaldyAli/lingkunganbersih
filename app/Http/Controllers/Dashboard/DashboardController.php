...
class DashboardController extends Controller
{
    public function index()
    {
        ...
        if (auth()->user()->hasRole('masyarakat')) {
            $data = (object) [
                'total_laporan' => number_format(Laporan::where('user_id', auth()->id())->count(), 0, ',', '.'),
                'total_dikirim' => number_format(Laporan::where('user_id', auth()->id())->where('status', 'DIKIRIM')->count(), 0, ',', '.'),
                'total_disetujui' => number_format(Laporan::where('user_id', auth()->id())->where('status', 'DISETUJUI')->count(), 0, ',', '.'),
                'total_ditolak' => number_format(Laporan::where('user_id', auth()->id())->where('status', 'DITOLAK')->count(), 0, ',', '.'),
            ];
        }
